import {
   Select,TextField, MenuItem, Grid, FormControl, InputLabel, FormHelperText
  ...
  {!addFormOpen && <Grid item xs={6}>
   <FormControl variant="filled" className={classes.formControl}>
        ...
        >
          ...
          {
            selectMenuItems.map(({name, id}) => <MenuItem key={id} value={id}>{name}</MenuItem>)
          }
        </Select>
